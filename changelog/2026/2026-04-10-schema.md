...
- **Read**: `schema.custominstance_read`, or `custom.{lowerCaseType}_read`, or `custom.{lowerCaseType}_read_own`
- **Manage**: `schema.custominstance_manage`, or `custom.{lowerCaseType}_manage`, or `custom.{lowerCaseType}_manage_own`
...
When a new custom schema type is created the platform provisions scopes for that type so clients can read and manage custom instances of that type using the `custom.{lowerCaseType}_*` patterns above.
