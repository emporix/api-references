...
# 2026-03-13: Order Service - checkout property for sales orders

## Overview

The sales order resource has been extended with a new `checkout` boolean field. This field:

- Can be provided only when creating an order
- Is available in tenant-managed order responses only (not in customer-managed order responses)
