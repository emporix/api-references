...
* `productType` — can be set to either `BASIC`, `BUNDLE`, `PARENT_VARIANT` or`VARIANT`. The productType value is immutable - once set, it cannot be changed during the product update.
...
The `pdf-mashup` service allows you to define product bundles in a new way. When the `product` is of the BUNDLE type (`productType: BUNDLE`) and contains products' ids in the `bundledProducts` fields, the `pdf-mashup` service generates the packlist.pdf and picklist.pdf files that list all products constituting the bundle.
