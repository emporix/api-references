...
Price lists are site-specific. You can restrict a price list to particular groups of customers with the `countries`, `regions`, and `customerGroups` fields, assign it to a specific legal entity with the `legalEntityId` field, or make the list time-limited with the `validity` field.

When a price list is assigned to a legal entity using `legalEntityId`, it automatically applies to all subsidiaries of that legal entity.
...
{% hint style="warning" %}
Customer groups cannot be provided if a legal entity is specified for the price list. You can use either `customerGroups` or `legalEntityId`, but not both.
{% endhint %}
...
curl -i -X POST \
  ...
  -d '{
    "name": {
      "en": "Company-specific price list"
    },
    "currency": "EUR",
    "countries": [
      "DE"
    ],
    "legalEntityId": "company_abc",
    "siteCode": "main",
    "validity": {
      "from": "2022-05-01T00:00:00.000Z",
      "to": "2025-05-01T00:00:00.000Z"
...
* `legalEntityId` — Legal entity for which price lists should be included in the price match. When specified, the price matching functionality will include price lists assigned to the specified legal entity as well as price lists assigned to any parent legal entities in the hierarchy.
