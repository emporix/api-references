...
When a customer sends a return request, the [Creating a single return entity](https://developer.emporix.io/api-references/orders/returns/api-reference/returns#post-return-tenant-returns) endpoint is called.
...
To create a return on behalf of a customer, you need to send a request to the [Creating a single return entity](https://developer.emporix.io/api-references/orders/returns/api-reference/returns#post-return-tenant-returns) endpoint.
...
Customers can update their return request only when the approval status of the request is `PENDING`. When a customer updates a return, the [Updating a single return](https://developer.emporix.io/api-references/orders/returns/api-reference/returns#post-return-tenant-returns) endpoint is called.
...
As an employee, you can update all the fields available during the return creation, as well as the `approvalStatus` and `received` fields, by sending a request to the [Updating a single return](https://developer.emporix.io/api-references/orders/returns/api-reference/returns#put-return-tenant-returns-returnid) endpoint.
