...
## Using `compoundLogicalQuery`

The special operator `compoundLogicalQuery` (for now available only in `product-service`) allows to compound complex queries with nested `OR` and `AND` operators. The syntax is following: **q=compoundLogicalQuery:((_expression1_) OR/AND (_expression2_))**.
